...
class Playlist {
  ...
  static findAll(userId) {
    return Playlist.findAllByUserAndChannel(userId, null);
  }

  static findAllByUserAndChannel(userId, channelId = null) {
    return new Promise((resolve, reject) => {
      let queryBody = `FROM playlists p WHERE p.user_id = ? `;
      const params = [userId];

      if (channelId === 'general') {
        queryBody += 'AND p.youtube_channel_id IS NULL ';
      } else if (channelId) {
        queryBody += 'AND p.youtube_channel_id = ? ';
