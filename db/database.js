...
function createTables() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      // Auto-Migration for YouTube Rotation Reuse
      db.run(`ALTER TABLE stream_rotations ADD COLUMN youtube_broadcast_id TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_broadcast_id to stream_rotations:', err.message);
        }
      });
      db.run(`ALTER TABLE stream_rotations ADD COLUMN youtube_stream_id TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding youtube_stream_id to stream_rotations:', err.message);
        }
      });
      db.run(`ALTER TABLE stream_rotations ADD COLUMN rtmp_url TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding rtmp_url to stream_rotations:', err.message);
        }
      });
      db.run(`ALTER TABLE stream_rotations ADD COLUMN stream_key TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding stream_key to stream_rotations:', err.message);
        }
      });
      ...
      db.run(`CREATE TABLE IF NOT EXISTS app_settings (
        ...
        // Run independent schema updates inside the final callback or before resolve
        db.run(`ALTER TABLE videos ADD COLUMN youtube_channel_id TEXT`, (err) => {
          if (err && !err.message.includes('duplicate column name')) console.error(err.message);
          db.run(`ALTER TABLE media_folders ADD COLUMN youtube_channel_id TEXT`, (err) => {
            if (err && !err.message.includes('duplicate column name')) console.error(err.message);
