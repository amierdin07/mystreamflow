    ...
    const initialStreamsData = null;
    const initialPaginationData = null;
    ...
    document.getElementById('newStreamForm').addEventListener('submit', function (e) {
      ...
      if (currentStreamMode === 'auto') {
        ...
        ytFormData.ytMonetization = document.getElementById('ytEnableMonetization').checked;
    ...
    function refreshStreamsView(page = currentPage, search = currentSearch) {
      currentPage = page;
      ...
      fetchStreams(page, currentLimit, search);
    }
    ...
    function createMobileStreamCard(stream) {
      ...
      if (stream.status === 'live') {
        const viewerBadge = stream.youtube_broadcast_id ? `<span class="flex items-center mt-1 text-green-400 text-[10px]"><i class="ti ti-eye mr-0.5"></i> <span class="concurrent-viewers" data-channel-id="${stream.youtube_channel_id}" data-broadcast-id="${stream.youtube_broadcast_id}">...</span></span>` : '';
        statusBadge = `<div><span class="flex items-center text-red-400"><span class="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse mr-1"></span><span class="text-[10px] font-medium">Live${durationDisplay ? ' • ' : ''}</span>${durationDisplay ? `<span class="text-[10px] font-medium tabular-nums live-duration" data-start-time="${stream.start_time}">${durationDisplay}</span>` : ''}</span>${viewerBadge}</div>`;
      ...
      if (stream.status === 'live' && stream.start_time) {
        ...
        statusBadge = `
          <div class="flex flex-col gap-1.5">
            <span class="flex items-center w-fit bg-red-400/10 text-red-400 rounded-full px-2.5 py-1">
              <span class="w-1.5 h-1.5 rounded-full bg-red-400 animate-pulse mr-1.5"></span>
            ...
            <span class="flex items-center w-fit bg-gray-800 text-green-400 rounded-full px-2.5 py-1 text-xs border border-gray-700">
              <i class="ti ti-eye mr-1"></i> <span class="concurrent-viewers ml-1" data-channel-id="${stream.youtube_channel_id}" data-broadcast-id="${stream.youtube_broadcast_id}">...</span>
            </span>` : ''}
          </div>
        `;
      } else {
    ...
    function openEditStreamModal(stream) {
      ...
      if (isYoutubeStream) {
        ...
        document.getElementById('editYtEnableMonetization').checked = stream.youtube_monetization === true || stream.youtube_monetization === 1;
  ...
  `;
      ...
      if (editForm) {
        editForm.addEventListener('submit', function (e) {
          ...
          if (currentEditStreamMode === 'auto') {
            ...
            ytFormDataObj.ytMonetization = document.getElementById('editYtEnableMonetization').checked;
    ...
    let dashboardServerTimezoneOffset = null;
    ...
    async function fetchServerTimeSync() {
      try {
        ...
        if (data.timezoneOffset !== undefined) {
          dashboardServerTimezoneOffset = data.timezoneOffset;
        }
      } catch (error) {
    ...
    function convertToServerTime(date) {
      if (dashboardServerTimezoneOffset === null) return date;
      const localOffset = date.getTimezoneOffset();
      const offsetDiff = localOffset - dashboardServerTimezoneOffset;
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      setInterval(() => {
        pollConcurrentViewers();
      }, 180000); // 3 minutes interval
    });

    async function pollConcurrentViewers() {
      const viewerElements = document.querySelectorAll('.concurrent-viewers');
      viewerElements.forEach(async (el) => {
        const channelId = el.getAttribute('data-channel-id');
        const broadcastId = el.getAttribute('data-broadcast-id');
        if (channelId && broadcastId) {
          try {
            const res = await fetch(`/api/youtube/live-viewers/${channelId}/${broadcastId}`);
            const json = await res.json();
            if (json.success && json.concurrentViewers !== undefined) {
              el.textContent = new Intl.NumberFormat().format(json.concurrentViewers);
            } else {
              el.textContent = 'Err';
            }
          } catch (e) {}
    ...
    // Call once initially after a small delay to let rows render
    setTimeout(pollConcurrentViewers, 2000);
