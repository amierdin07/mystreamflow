...
async function startRotationStream(rotation, item) {
  try {
    ...
    let broadcastId = rotation.youtube_broadcast_id;
    let liveStreamId = rotation.youtube_stream_id;
    let rtmpUrl = rotation.rtmp_url;
    let streamKey = rotation.stream_key;

    if (broadcastId) {
      try {
        console.log(`[RotationService] Reusing existing YouTube broadcast: ${broadcastId}`);
        await youtube.liveBroadcasts.update({
          ...
          requestBody: {
            ...
            contentDetails: {
              ...
              enableAutoStop: false // Keep it open for rotation
            }
          }
        });
      } catch (updateError) {
        console.warn(`[RotationService] Failed to reuse broadcast ${broadcastId}, creating new one. Error: ${updateError.message}`);
        broadcastId = null;
      }
    }
    ...
    if (item.youtube_monetization) {
      try {
        await syncBroadcastMonetization(youtube, broadcastId, true);
      } catch (monetizationError) {
        console.warn(`[RotationService] Failed to enable monetization for broadcast ${broadcastId}. Error: ${monetizationError.message}`);
      }
    }
    ...
    if (!liveStreamId || !rtmpUrl || !streamKey) {
      const streamResponse = await youtube.liveStreams.insert({
        ...
        requestBody: {
          snippet: {
            title: `Rotation Stream: ${rotation.name}`
          },
          cdn: {
            frameRate: '30fps',
            ingestionType: 'rtmp',
      ...
      // Save to rotation model for future reuse
      await Rotation.update(rotation.id, {
      ...
      // Even if reusing, we might need to re-bind if the broadcast was new
      try {
        await youtube.liveBroadcasts.bind({
          part: ['id', 'contentDetails'],
          id: broadcastId,
        ...
        // If already bound, it might throw, which is fine
      }
    }
    ...
    const stream = await Stream.create({
      ...
      resolution: item.resolution || rotation.resolution || '1280x720',
      bitrate: item.bitrate || rotation.bitrate || 2500,
      fps: item.fps || rotation.fps || 30,
      status: 'scheduled',
    ...
    return { success: true, streamId: stream.id, broadcastId: broadcastId };
  } catch (error) {
    const betterError = youtubeService.handleYoutubeError(error, `(Rotation: ${rotation.name})`);
    console.error('[RotationService] Error starting rotation stream:', betterError.message);
    return { success: false, error: betterError.message, code: betterError.code || null };
  }
}
...
async function stopRotationStream(rotation, item) {
  try {
    ...
    if (rotationStream) {
      ...
      // completing it would cost extra quota and prevent reuse.
    }
